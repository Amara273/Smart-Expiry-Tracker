...
class SmartGroceryTrackerUI:
    def __init__(self, root, filename='grocery_data.csv'):
        ...
        root.grid_rowconfigure(1, weight=1)      # Allow row 1 (main frame) to expand
        ...
        # Dropdown Menu
        self.function_var = ctk.StringVar(value="Menus")
        ...
        self.update_frame = None  # Added this to track the update frame
        self.minus_btn = None     # Added this to track the minus button
        self.plus_btn = None      # Added this to track the plus button
        
        # Add Exit Button to the main window
        # Change this line in the __init__ method:
        exit_button = ctk.CTkButton(root, 
                         text="Exit", 
                         fg_color="#bf3a3a",  # Red color
                         hover_color="#8f2b2b",  # Darker red on hover
                         command=self.on_closing)  # Use on_closing instead of root.destroy
    ...
    def load_data(self):
        ...
        if os.path.exists(self.filename):
            ...
            # Remove any unwanted columns (like Days_To_Expiry)
    ...
    def display_items(self):
        ...
        if self.df.empty:
            ...
            for _, row in self.df.iterrows():
                ...
                # Create formatted line with adjusted column widths and no fixed width for the last column
    ...
    def search_item(self):
        ...
        if results.empty:
            ...
            self.textbox.insert("end", tabulate(results, headers='keys', tablefmt='grid'))

    def handle_function(self, choice):
        """Calls the selected function from the dropdown menu."""
        # Clear any existing update UI elements
    ...
    def clear_update_ui(self):
        ...
        if self.update_frame:
            self.update_frame.destroy()  # Destroy the entire frame with all its children
    ...
    def add_item(self):
        ...
        # Create a frame for the calendar but don't display it initially
    ...
    def check_low_stock(self):
        ...
        try:
            ...
            if low_stock_items.empty:
                ...
                # Display each low stock item in a clean format
                for _, row in low_stock_items.iterrows():
                    item_name = row['Name']
    ...
    def sort_items(self):
        ...
        ctk.CTkButton(
            ...
            fg_color="#bf3a3a",  # Different color to highlight importance
            hover_color="#8f2b2b"
        ).pack(pady=5, fill="x", padx=10)
    ...
    def filter_items(self):
        ...
        filter_dialog.grab_set()  # Make it modal
    ...
    def update_shopping_list(self):
        ...
        # If there are multiple items with the same name, ask which one to update
       # If there are multiple items with the same name, ask which one to update
        if len(name_matches) > 1:
            ...
            # Format the options in a cleaner way without grid lines
            options_text = ""
            ...
            # Create a more appealing dialog
